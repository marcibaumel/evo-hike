...
const getTranslationKey = (text: string): string | null => {
    ...
    if (cleanText.includes('esokabat') || cleanText.includes('valtoruha')) return 'raincoat';

    if (cleanText.includes('telefon') || cleanText.includes('powerbank')) return 'phone';

    if (cleanText.includes('terkep') || cleanText.includes('gps')) return 'map';

    if (cleanText.includes('elsosegely')) return 'first_aid';

    if (cleanText.includes('elelem') || cleanText.includes('energia')) return 'food';
        ...
        <div style={overlayStyle}>
            <div style={contentStyle}>
                <h2 style={{ marginTop: 0, textAlign: 'center', color: '#333' }}>
                    {t('plan.title')}: <br />
                    <span style={{ fontSize: '0.8em', color: '#666', fontWeight: 'normal' }}>{trailName}</span>
                </h2>
                ...
                {success ? (
                    ...
                    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        ...
                        <div>
                            <h4 style={{ marginBottom: '10px', color: '#333' }}>{t('plan.checklist_title')}</h4>

                            {checklistOptions.length === 0 && !error && <p style={{ color: '#666' }}>Bet�lt�s...</p>}
                        ...
                        <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
                            <div style={{ flex: 1 }}>
                                <Button onClick={() => {}}>{loading ? t('plan.saving') : t('plan.save')}</Button>
                            </div>
                            <button
                                type="button"
                                onClick={onClose}
