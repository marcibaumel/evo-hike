...
import { FileTextIcon, XIcon, CameraPlusIcon, CheckCircleIcon } from '@phosphor-icons/react';
    ...
    return (
        <div className="route-form-container">
            <input
                ...
                accept="image/*"
            />
            <input type="file" ref={gpxInputRef} onChange={handleGpxChange} className="route-form-ref" accept=".gpx" />
            ...
            <form className="route-form-card" onSubmit={handleSubmit}>
                <div className="route-form-top-row">
                    ...
                    <button className="route-form-photo-box" onClick={triggerFileInput}>
                        {photo && (
                            <button type="button" className="route-form-photo-remove" onClick={handleRemovePhoto}>
                                <XIcon size={14} />
                            </button>
                        )}
                        <div className="route-form-photo-icon">
                            {photo ? <CheckCircleIcon size={24} color="#22c55e" /> : <CameraPlusIcon size={24} />}
                        </div>
                        <span className="route-form-photo-name">{photo ? photo.name : t('routeForm.add_photo')}</span>
                    </button>
                </div>
                ...
                <div className="route-form-dynamic-section">
                    ...
                    {stops.length < 5 && (
                        <button type="button" className="route-form-add-btn" onClick={addStop}>
                            {t('routeForm.poi.new')}
                        </button>
                    )}
                </div>
