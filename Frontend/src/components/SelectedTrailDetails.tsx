    ...
    return (
        <div className="selected-trail-details">
            ...
            <p>
                ...
                <strong>{t('selectedTrail.elevation')}:</strong> {trail.elevationGain}m |{' '}
                <strong>{t('selectedTrail.time')}:</strong> {Math.floor(trail.time / 60)} {t('selectedTrail.hour')}{' '}
            ...
            {pois.length > 0 && (
                <div>
                    <h3 className="trail-section-header">
                        ...
                        />{' '}
                        {t('selectedTrail.nearby_pois')} ({pois.length} {t('selectedTrail.items')}):
                    </h3>
                            ...
                            <li key={poi.id} className="trail-poi-item">
                                <button
                                    ...
                                    <small className="trail-poi-type">
                                        ({poi.tags?.tourism || poi.tags?.natural || poi.tags?.historic})
                                    </small>
                                </button>
                            </li>
                        ))}
