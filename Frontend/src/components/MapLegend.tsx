...
export default function MapLegend() {
    ...
    if (!isOpen) {
        return (
            <button className="map-legend-toggle-btn" onClick={() => setIsOpen(true)} title={t('mapLegend.title')}>
    ...
    return (
        <div className="map-legend-container">
            ...
            <div className="legend-section">
                ...
                <LegendItem icon={<MdTerrain />} color="#795548" label={t('mapLegend.peak')} />
                <LegendItem icon={<MdWaterDrop />} color="#2196F3" label={t('mapLegend.spring')} />
                <LegendItem icon={<GiCaveEntrance />} color="#424242" label={t('mapLegend.cave')} />
                <LegendItem icon={<GiWaterfall />} color="#00BCD4" label={t('mapLegend.waterfall')} />
            </div>

            {/* turista dolgok */}
            <div className="legend-section">
                <h4 className="legend-section-title">{t('mapLegend.tourism')}</h4>
                <LegendItem icon={<MdVisibility />} color="#FF9800" label={t('mapLegend.viewpoint')} />
                <LegendItem icon={<MdPlace />} color="#F44336" label={t('mapLegend.attraction')} />
                <LegendItem icon={<MdMuseum />} color="#8D6E63" label={t('mapLegend.museum')} />
            </div>

            {/* történelmi cuccok */}
            <div className="legend-section">
                <h4 className="legend-section-title">{t('mapLegend.historic')}</h4>
                <LegendItem icon={<GiCastle />} color="#9C27B0" label={t('mapLegend.castle')} />
                <LegendItem icon={<GiBrokenWall />} color="#757575" label={t('mapLegend.ruins')} />
                <LegendItem icon={<MdChurch />} color="#607D8B" label={t('mapLegend.monument')} />
            </div>

            {/* szolgáltatások */}
            <div>
                <h4 className="legend-section-title">{t('mapLegend.service')}</h4>
                <LegendItem icon={<MdLocalDrink />} color="#03A9F4" label={t('mapLegend.drinking_water')} />
                <LegendItem icon={<MdChurch />} color="#673AB7" label={t('mapLegend.church')} />
                <LegendItem icon={<MdRestaurant />} color="#E91E63" label={t('mapLegend.restaurant')} />
            </div>
        </div>
    );
}
