...
export const getIconForPoi = (poi: OverpassElement) => {
    ...
    if (tags.natural === 'peak' || tags.natural === 'saddle') return createReactIcon(<MdTerrain />, '#795548');
    if (tags.natural === 'spring') return createReactIcon(<MdWaterDrop />, '#2196F3');
    if (tags.natural === 'cave_entrance') return createReactIcon(<GiCaveEntrance />, '#424242');
    if (tags.waterway === 'waterfall' || tags.natural === 'waterfall')
        return createReactIcon(<GiWaterfall />, '#00BCD4');

    if (tags.tourism === 'viewpoint') return createReactIcon(<MdVisibility />, '#FF9800');
    if (tags.tourism === 'attraction') return createReactIcon(<MdPlace />, '#F44336');
    if (tags.tourism === 'museum') return createReactIcon(<MdMuseum />, '#8D6E63');

    if (tags.historic === 'castle') return createReactIcon(<GiCastle />, '#9C27B0');
    if (tags.historic === 'ruins') return createReactIcon(<GiBrokenWall />, '#757575');
    if (tags.historic === 'memorial' || tags.historic === 'monument') return createReactIcon(<MdChurch />, '#607D8B');

    if (tags.amenity === 'drinking_water') return createReactIcon(<MdLocalDrink />, '#03A9F4');
    if (tags.amenity === 'place_of_worship') return createReactIcon(<MdChurch />, '#673AB7');
    if (tags.amenity === 'restaurant') return createReactIcon(<MdRestaurant />, '#E91E63');
